...
This repository contains the Raspberry Pi software stack (RaspberryPi/) and user application software stack (FallWatch/) for the Faceplant Forecast Human Activity Detector. It also has the unused radar firmware from ECEN 403 (Firmware/). The FALLWATCH system uses a **TI AWR2944EVM millimeter-wave radar module** to continuously monitor a room, runs radar data through a custom-trained AI model to detect fall events, and automatically pushes notifications to a cloud backend when a fall is detected.
